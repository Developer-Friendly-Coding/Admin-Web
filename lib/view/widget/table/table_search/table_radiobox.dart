import 'package:clean_arch/provider/impl/table_provider_impl.dart';
import 'package:flutter/material.dart';
import 'package:clean_arch/model/base_model.dart';
import 'package:provider/provider.dart';

class TableRadioBox<M extends Base> extends StatefulWidget {
  final double height;
  final double width;
  final Color color;
  final bool test;
  final List<Enum> enumList;
  final double iconScale;
  final double iconLeftMargin;
  final double textLeftMargin;
  final String memberName;
  const TableRadioBox(
      {required this.enumList,
      required this.memberName,
      this.width = 110,
      this.height = 35,
      this.test = false,
      this.color = Colors.transparent,
      this.iconScale = 0.7,
      this.textLeftMargin = 27,
      this.iconLeftMargin = 0,
      super.key});

  @override
  State<TableRadioBox<M>> createState() => _TableRadioBoxState<M>();
}

class _TableRadioBoxState<M extends Base> extends State<TableRadioBox<M>> {
  String? enumValueString;
  List<Widget> radioListTile(BuildContext context) {
    TableProvider<M> providerRead =
        Provider.of<TableProvider<M>>(context, listen: false);

    List<Widget> result = [];

    for (int i = 0; i < widget.enumList.length; i++) {
      Container radioListTile = Container(
        color: widget.test == true ? Colors.amber : null,
        width: widget.width,
        height: widget.height,
        child: Stack(
          children: [
            Align(
              alignment: Alignment.centerLeft,
              child: Container(
                margin: EdgeInsets.only(left: widget.iconLeftMargin),
                child: Transform.scale(
                  scale: widget.iconScale,
                  child: Radio(
                    value: widget.enumList[i].toString(),
                    groupValue: enumValueString,
                    onChanged: (String? value) {
                      setState(() {
                        enumValueString = value;
                      });
                      providerRead.getDataByRadioBox(
                          widget.memberName, widget.enumList[i]);
                    },
                  ),
                ),
              ),
            ),
            Align(
                alignment: Alignment.centerLeft,
                child: Container(
                    margin: EdgeInsets.only(left: widget.textLeftMargin),
                    child: Text(widget.enumList[i].toString())))
          ],
        ),
      );

      result.add(radioListTile);
    }

    result.add(Container(
      color: widget.test == true ? Colors.amber : null,
      width: widget.width,
      height: widget.height,
      child: Stack(
        children: [
          Align(
            alignment: Alignment.centerLeft,
            child: Container(
              margin: EdgeInsets.only(left: widget.iconLeftMargin),
              child: Transform.scale(
                scale: widget.iconScale,
                child: Radio(
                  value: "모두",
                  groupValue: enumValueString,
                  onChanged: (String? value) {
                    setState(() {
                      enumValueString = value;
                    });
                    providerRead.getDataByRadioBox(widget.memberName, null);
                  },
                ),
              ),
            ),
          ),
          Align(
              alignment: Alignment.centerLeft,
              child: Container(
                  margin: EdgeInsets.only(left: widget.textLeftMargin),
                  child: const Text("모두")))
        ],
      ),
    ));
    return result;
  }

  @override
  Widget build(BuildContext context) {
    return Container(
      color: widget.color,
      child: Wrap(
        direction: Axis.horizontal,
        children: radioListTile(context),
      ),

      //
    );
  }
}
